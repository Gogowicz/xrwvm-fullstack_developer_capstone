...
const Dealer = () => {
  ...
  // *** FIX 1: root_url stabil und korrekt ***
  ...
  const get_dealer = async () => {
    try {
      ...
      if (retobj.status === 200) {
        setDealer(retobj.dealer);   // FIX: direkt setzen
      }
    } catch (err) {
  ...
  // *** FIX 2: Dealer noch nicht geladen → Loading ***
  if (!dealer) {
    return (
      <div style={{ margin: "20px" }}>
        <Header />
        <h2 style={{ color: "grey" }}>Loading dealer...</h2>
      </div>
    );
  }
