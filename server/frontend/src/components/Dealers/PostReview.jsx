...
const PostReview = () => {
  ...
  const postreview = async () => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      alert("Please select a valid purchase date using the date picker.");
      return;
    }
    ...
    if (!/^\d{4}$/.test(year) || year < 2015 || year > 2026) {
      alert("Please enter a valid car year (2015–2026).");
      return;
    }
  ...
  if (!dealer) {
    return (
      <div>
        <Header />
        <h2 style={{ margin: "5%", color: "grey" }}>Loading dealer...</h2>
      </div>
    );
  }
